...
<div class="flex flex-col z-20 w-full py-2 px-4 fixed items-center bg-ivory" transition:slide>
    {#each options as item}
        <div class="flex flex-row w-full h-10 items-center">
            ...
            checked:bg-grass checked:border-4 checked:border-pine hover:cursor-pointer
            ...
            />
            <label class="text-cocoa font-bold hover:cursor-pointer" for="option-{item.id}">{item.name}</label>
        </div>
    {/each}
