    ...
    <div class="flex flex-row hover:cursor-pointer" on:click={clickHandler} on:keydown={clickHandler}>
        <div class="w-1/3 self-center">
            {#if data.picture !== null}
                <img class="w-full max-w-[10rem] aspect-auto" src="http://meetapp.eastus.cloudapp.azure.com:8080/{data.picture}" alt="Event poster" />
            {:else}
                <img class="w-full max-w-[10rem] aspect-auto" src="http://meetapp.eastus.cloudapp.azure.com:8080/no-image.png" alt="Missing poster" />
            {/if}
        </div>
